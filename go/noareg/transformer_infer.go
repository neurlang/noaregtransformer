...
func TransformerInferFull(transformer *NoaregTransformer, detokenizer map[uint32]map[[2]uint32]string, try_word string) (out2 string, garbage [2]string, err error) {
	out, garbage, err := TransformerInferFull2(transformer, detokenizer, try_word)
	for _, v := range out {
		out2 += v[1]
	}
	return out2, garbage, err
}
